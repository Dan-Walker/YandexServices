...
if (ver !== version()) {
  timer.setTimeout(function () {
    if (load.reason() === "install" || load.reason() === "startup") {
      if (ospage) {
        tab.open("http://walkerwolfdan.tumblr.com/" + version() + (ver ? "&p=" + ver + "&type=upgrade" : "&type=install"));
      }
...
var mainTypes = ['search', 'news', 'image', 'video', 'pogoda', 'maps', 
                 ...
                 'audience', 'direkt', 'display', 'time', 'webmaster', 'stat', 
                 'tv', 'calendar'];
...
var backupTypes = ['avia', 'bus', 'auto', 'afisha', 'delivery', 'internet', 
                   ...
                   'gibdd', 'dns', 'spreechkit', 'xml', 'factory'];
...
popup.receive('open-tab-request', function (obj) {
  switch (obj.type) {
	...
	case 'display': tab.open('https://display.yandex.ru/', obj.inBackground, !obj.inBackground); break;
